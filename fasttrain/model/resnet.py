...
class ResNetCIFAR(nn.Module):
    def __init__(self, n, pre_activated=False, stochastic_depth=False):
        ...
        for i in range(n):
            self.seq32_32.add_module(str(i), SimpleBlock(16, prob=prob))

        self.seq16_16 = nn.Sequential()
        for i in range(n):
            if i == 0:
                self.seq16_16.add_module(str(i), DownBlock(16, prob=prob))
            else:
                self.seq16_16.add_module(str(i), SimpleBlock(32, prob=prob))

        self.seq8_8 = nn.Sequential()
        for i in range(n):
            if i == 0:
                self.seq8_8.add_module(str(i), DownBlock(32, prob=prob))
            else:
                self.seq8_8.add_module(str(i), SimpleBlock(64, prob=prob))
